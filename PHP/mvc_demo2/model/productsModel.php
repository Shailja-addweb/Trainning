	...
	class productsModel {
		...
		function __construct()
	  	...
	  	public function ProductList() {
	  		$query = "SELECT p.id, p.name,GROUP_CONCAT(DISTINCT c.name),
	  					...
	  					WHERE p.isdelete IS NULL AND c.isdelete IS NULL  
	  					GROUP BY p.id";
		...
		public function InsertImage($arrayimage,$p_id) {
	  	...
	  	public function FetchProductDetails($id) {
	  		$query = "SELECT p.id, p.name,GROUP_CONCAT(DISTINCT c.name), 
	  					...
	  					INNER JOIN product_category AS pc ON p.id = pc.p_id 
	  					INNER JOIN category AS c ON pc.c_id = c.id 
	  					WHERE p.isdelete IS NULL AND c.isdelete IS NULL AND p.id = $id
	  					GROUP BY p.id" ;		
			$result = mysqli_query($this->con, $query);
			return $result;
	  	}
	  	...
	  	public function FetchName($key) {
	  		$query = "SELECT id,name FROM images 
	  					WHERE id = $key AND isdelete IS NULL  AND name <> 'default.png' ";
	  		$result = mysqli_query($this->con, $query);
	  		$noofrow = mysqli_num_rows($result);  
            if($noofrow>0) {
