...
<form method="post" action="" name="addemployeeForm" id="emp-form" enctype="multipart/form-data"> 
	<table cellpadding="10">
		...
		<tr>
			<td>Image:</td>
			<td>
				<div id = "image"><?php if (!empty($row['id'])) { ?>
						<div class="forImages">
							<div class="images">
					...
					<a href="javascript:;" id="newimage">Add Image</a> 
					 &nbsp; &nbsp;  
					<a id="remove" data-id=<?php echo $row['id'];?> href="javascript:;">REMOVE ALL IMAGES</a></div><br>

				<?php } 

				else { ?>
					<div class="images"><input type="file" name="image[]" id="image"  value="<?php if(!empty($row['GROUP_CONCAT(DISTINCT i.name)'])) echo $row['GROUP_CONCAT(DISTINCT i.name)']; else echo '';?>" multiple ></div> <br>
